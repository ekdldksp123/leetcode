...
const merge = (intervals) => {
    ...
    for(const current of intervals) {
        if(result.length === 0 || result[result.length-1][1] < current[0]) {
            result.push(current);
        } else if(result[result.length-1][1] >= current[0] || result[result.length-1][1] > current[0]) {
...
* 3. prev[0] <= current[1] 이면 result[result.length - 1] = Math.max(current[1],result[result.length]);
*/